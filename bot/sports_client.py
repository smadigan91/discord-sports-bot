...
class SportsClient(discord.Client):
    ...
    async def handle_basketball_request(self, message):
        sport = 'nba'
        content_lower = message.content.lower()
        # /blurb [firstname]* [lastname]*
        if content_lower.startswith('/blurb'):
        ...
        # /log [player]*
            ...
            try:
                ...
                await self.send_message(message.channel, embed=embedded_stats)
            except Exception as ex:
                raise ex
            ...
            try:
                await self.do_bball_highlight(channel=message.channel)
            except Exception as ex:
                raise ex
        elif content_lower.startswith('/lowlight'):
            try:
                await self.do_bball_lowlight(channel=message.channel)
            except Exception as ex:
                raise ex

    async def handle_baseball_request(self, message):
        sport = 'mlb'
        # /help
        content_lower = message.content.lower()
        if content_lower.startswith('/help'):
            try:
                ...
                await self.send_message(message.channel, embed=help_map)
            except Exception as ex:
                raise ex
        ...
        if content_lower.startswith('/highlight'):
            ...
            try:
                if msg[0] == 'index':
                    ...
                    await self.send_message(message.channel, embed=highlights)
                elif msg[-1].isdigit():
                    index = msg[-1]
                    search = '%2B'.join(msg[:-1])
                    highlight = get_highlight(search, int(index) - 1)
                    await self.send_message(message.channel, content=response % highlight)
                else:
    ...
    async def handle_blurb(self, message, sport):
        msg = message.content.split()[1:]
        try:
            search = " ".join(msg[0:])
            if not search:
